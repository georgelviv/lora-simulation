...
def path_loss_hata(
    ...
    f_mhz = freq_hz / 1e6
    d_km = max(distance_m / 1000.0, 0.01)  # avoid singularity
...
def compute_rssi_hata(
    ...
    rssi = tx_power_dbm - pl + shadowing
...
def lora_rssi_hata_chip(
    ...
    area: AreaType,
    *,
    reporting_noise_db: float = 0.8,  # ≤ 1 dB
) -> float:
    ...
    chip_noise = random.uniform(-reporting_noise_db, reporting_noise_db)

    return round(rssi_physical + chip_noise, 2)
